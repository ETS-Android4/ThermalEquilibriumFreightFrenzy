 ...
 */
@Config
public class BenLocalizerRR extends ThreeTrackingWheelLocalizer {
	...
	public BenLocalizerRR(HardwareMap hardwareMap) {
		super(Arrays.asList(
		...
		this.leftSensor = hardwareMap.get(Rev2mDistanceSensor.class, "LeftDistance");
		this.rightSensor = hardwareMap.get(Rev2mDistanceSensor.class, "RightDistance");
		this.rearSensor = hardwareMap.get(Rev2mDistanceSensor.class, "RearDistance");
		timer.reset();
	...
	public void update() {
		...
		if (Math.abs(AngleWrap(getPoseEstimate().getHeading())) > maximumAngle) return;

		double x = rearDistance * Math.cos(getPoseEstimate().getHeading());
		double y = leftDistance * Math.cos(getPoseEstimate().getHeading());
